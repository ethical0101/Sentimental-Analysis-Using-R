...
#' @export
run_complete_analysis <- function(file_path = NULL, run_ml = TRUE, k_topics = 3) {
  ...
  cat("[STEP 2/9] TEXT PREPROCESSING\n")
  cat("--------------------------------------------\n")
  ...
  if (run_ml) {
    cat("\n[STEP 8/9] MACHINE LEARNING CLASSIFICATION\n")
    cat("--------------------------------------------\n")
  ...
  if (!is.null(topic_results)) {
    ...
    if (nrow(topic_sentiment_analysis) > 0) {
      ...
      cat(sprintf("🔍 Topic with Highest Negativity: Topic %d (%.1f%% negative)\n",
  ...
  # Peak negative date
  if (ts_results$has_time_variation) {
    cat(sprintf("📉 Peak Negative Date: %s (Score: %.2f)\n",
  ...
  if (sarcasm_summary$stats$sarcastic_count > 0) {
    cat(sprintf("🎭 Sarcasm Detection Rate: %.1f%% (%d instances)\n",
