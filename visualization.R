...
plot_sentiment_trend <- function(daily_sentiment) {
    ...
    p <- ggplot(daily_sentiment, aes(x = date, y = avg_sentiment)) +
        ...
        labs(
            ...
            subtitle = paste(
                "Date Range:", min(daily_sentiment$date), "to",
...
#' Create Multi-Line Sentiment Trend by Polarity Counts
#' @param daily_sentiment Daily aggregated sentiment data
#' @return ggplot object
#' @export
plot_sentiment_trend_multiline <- function(daily_sentiment) {
    if (is.null(daily_sentiment) || nrow(daily_sentiment) == 0) {
        p <- ggplot() +
            annotate("text",
                x = 0.5, y = 0.5,
    ...
    trend_long <- daily_sentiment %>%
        select(date, positive_count, negative_count, neutral_count) %>%
        pivot_longer(
            cols = c(positive_count, negative_count, neutral_count),
            names_to = "sentiment_class",
            values_to = "count"
        ) %>%
        mutate(
            sentiment_class = recode(
                sentiment_class,
    ...
    p <- ggplot(trend_long, aes(x = date, y = count, color = sentiment_class)) +
        ...
        labs(
            title = "Sentiment Trend Over Time",
            subtitle = "Daily Positive, Negative, and Neutral counts",
            x = "Date",
            y = "Daily Count",
            color = "Sentiment"
        ) +
...
#' Create Sentiment Index Trend Plot
#' @param daily_sentiment Daily aggregated sentiment data
#' @return ggplot object
#' @export
plot_sentiment_index_trend <- function(daily_sentiment) {
    if (is.null(daily_sentiment) || nrow(daily_sentiment) == 0 ||
        !"sentiment_index" %in% colnames(daily_sentiment)) {
        p <- ggplot() +
            annotate("text",
                x = 0.5, y = 0.5,
                label = "No sentiment index data available", size = 6
            ) +
            theme_void()
        return(p)
    }

    p <- ggplot(daily_sentiment, aes(x = date, y = sentiment_index)) +
        geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
        geom_line(color = "#1f78b4", linewidth = 1.2) +
        geom_point(color = "#0b3c5d", size = 2.1) +
        geom_smooth(method = "loess", se = FALSE, color = "#e67e22", linetype = "dotted") +
        labs(
            title = "Daily Sentiment Index Trend",
            subtitle = "Sentiment_Index = (Positive_Count - Negative_Count) / Total_Count",
            x = "Date",
            y = "Sentiment Index"
        ) +
        theme_minimal() +
...
create_visualization_dashboard <- function(data, daily_sentiment = NULL,
    ...
    if (!is.null(daily_sentiment)) {
        ...
        p3b <- plot_sentiment_trend_multiline(daily_sentiment)
        ggsave("sentiment_trend_multiline.png", p3b, width = 10, height = 6)

        p3c <- plot_sentiment_index_trend(daily_sentiment)
        ggsave("sentiment_index_trend.png", p3c, width = 10, height = 6)
    }
